...
- Java 19 pour Java
- Compilateur C pour C
- Interpréteur Python pour Python

=== Reproductibilité

- Pour reproduire nos analyses :
. Récupérer RLERuntimeTest situé dans analyse
. L'insérer dans le dossier main\java\iut\sae\algo
. Si la classe fournie respecte les normes demandées, alors tout devrait fonctionner.

- Pour Python : 
. Copier coller le script fournie entre @pyRAPL.measure() et start_time
. Change RLE par unRLE dans la boucle pour tester l'un ou l'autre

=== Références

- link:https://github.com/powerapi-ng/pyRAPL[pyRAPL]
